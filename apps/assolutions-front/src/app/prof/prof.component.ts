...
import { ProfesseurVM, SeanceProfesseurVM, SeanceVM } from '@shared/src';
...
export class ProfComponent implements OnInit { 
  @Input() liste_prof:ProfesseurVM[]=[];
  @Input() manage = false;
  @Input() thisSeance:SeanceVM;
  @Input() Profs:SeanceProfesseurVM[] = [];
  current_prof_key:number;
  action:string = "";
  LVSeanceProf :KeyValuePair[];
  @Input() prof_dispo:ProfesseurVM[]=[];
  @Output() profUpdated = new EventEmitter<SeanceProfesseurVM[]>();  // Ajout du @Output()
  ...
  AjouterProf() {
    ...
    if(this.thisSeance.seance_id == 0){      
      this.Profs.push(S);
  ...
  MAJListeProf() {
    ...
    this.Profs.forEach((element: SeanceProfesseurVM) => {
