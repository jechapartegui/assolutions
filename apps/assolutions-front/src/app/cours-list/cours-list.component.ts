...
export class CoursListComponent {
  ...
  constructor(
    public readonly store: CoursStore,
  ...
  exportExcel(): void {
    ...
    const columns: ExcelColumn<Cours_VM>[] = [
      { header: $localize`:@@common.id:ID`, value: c => c.id },
      { header: $localize`:@@course.name:Cours`, value: c => c.nom },
      { header: $localize`:@@course.day:Jour`, value: c => c.jour_semaine },
      { header: $localize`:@@common.time:Heure`, value: c => c.heure },
      { header: $localize`:@@course.duration:Durée`, value: c => c.duree },
      { header: $localize`:@@place.name:Lieu`, value: c => c.lieu?.nom ?? '' },
      {
      ...
      },
      {
        ...
        value: c => this.vm.refs?.listeSaison?.find(s => s.id === c.saison_id)?.nom ?? '',
      },
      { header: $localize`:@@member.minAge:Âge minimum`, value: c => c.age_minimum },
      { header: $localize`:@@member.maxAge:Âge maximum`, value: c => c.age_maximum },
      { header: $localize`:@@place.maximum:Places maximum`, value: c => c.place_maximum },
      { header: $localize`:@@trial.allowed:Essai possible`, value: c => c.essai_possible },
      { header: $localize`:@@meeting.point:RDV`, value: c => c.rdv },
      { header: $localize`:@@attendance.display:Afficher présents`, value: c => c.afficher_present },
      { header: $localize`:@@convocation.nominative:Convocation nominative`, value: c => c.convocation_nominative },
    ];
  ...
  async deleteSelection(): Promise<void> {
    ...
    const confirmDelete = window.confirm(
      count === 1
        ? $localize`:@@course.deleteOne:Voulez-vous supprimer le cours sélectionné ?`
        : $localize`:@@course.deleteMany:Voulez-vous supprimer les ${count}:COUNT: cours sélectionnés ?`,
    );
  ...
  getJourLabel(jour: string): string {
    switch ((jour ?? '').toLowerCase()) {
      case 'lundi': return $localize`:@@weekday.monday:Lundi`;
      case 'mardi': return $localize`:@@weekday.tuesday:Mardi`;
      case 'mercredi': return $localize`:@@weekday.wednesday:Mercredi`;
      case 'jeudi': return $localize`:@@weekday.thursday:Jeudi`;
      case 'vendredi': return $localize`:@@weekday.friday:Vendredi`;
      case 'samedi': return $localize`:@@weekday.saturday:Samedi`;
      case 'dimanche': return $localize`:@@weekday.sunday:Dimanche`;
      default: return jour ?? '';
    }
  }
