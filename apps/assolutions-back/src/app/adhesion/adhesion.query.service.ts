  ...
  async GetAdherentAdhesion(saisonId: number, login: string): Promise<any[]> {
    const sql = `
      ...
      WHERE a.saison_id = $1
        ...
        AND p.archive = false
    `;
    return this.dataSource.query(sql, [saisonId, login]);
  }

  /**
   * Liste métier utilisée par un professeur pour convoquer manuellement une
   * personne à une séance. Le contrôle "staff du projet" est fait dans le
   * controller avant d'arriver ici et la saison est recroisée avec le projet.
   */
  async getAdherentsForSeason(saisonId: number, projectId: number): Promise<any[]> {
    const sql = `
      SELECT DISTINCT
        p.id,
        p.first_name AS prenom,
      ...
      JOIN inscription_saison ins
        ON ins.personne_id = p.id
       AND ins.active = true
      JOIN saison s
        ON s.id = ins.saison_id
      WHERE ins.saison_id = $1
        AND s.project_id = $2
        AND p.archive = false
      ORDER BY p.last_name, p.first_name, p.nickname
    `;

    return this.dataSource.query(sql, [saisonId, projectId]);
  }

  async getAnniversaire(saisonId: number): Promise<string[]> {
    const today = new Date();
    const sql = `
      SELECT p.first_name || ' ' || p.last_name AS nom
      FROM personne p
